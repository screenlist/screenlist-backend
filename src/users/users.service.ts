...
export class UsersService {
  ...
  async revokePrivilegedRole(uid: string, opt: UserOpt) {
    try {
      ...
      const { entity, history } = await this.db.updateUserEntity(
        updateUser,
        updateUserOptions,
      );
      return entity;
  ...
  async deleteUser(opt: UserOpt) {
    ...
    try {
      ...
      const profileForDeletionKey = this.db.key([
        'User',
        profileForDeletion.uid,
      ]);

      // Non admin users cannot delete other users' profiles
      if (
        currentUser.uid !== profileForDeletion.uid &&
      ...
      if (profileForDeletion.hasOwnProperty('mailId')) {
        await fetch(
          `https://api.brevo.com/v3/contacts/${profileForDeletion.mailId}`,
          {
            method: 'DELETE',
            headers: {
      ...
      await this.search.client.collections('users').documents(profileForDeletion.uid).delete();
  ...
  async updateUserEmail(opt: UserOpt) {
    ...
    try {
      ...
      if (!user.hasOwnProperty('mailId')) {
        ...
        await this.db.updateUserEntity(
          { mailId: createContactData.id, lastUpdated: opt.time },
          opt,
        );
        ...
        const updateContact = await fetch(
          `https://api.brevo.com/v3/contacts/${user.mailId}`,
          {
            method: 'PUT',
            headers: {
              accept: 'application/json',
  ...
  async passwordReset(email: string) {
    try {
      const [existing] = await this.db
        .createQuery('Fleet')
        .filter('passwordResetEmail', '=', email)
        .run();
      if (existing.length > 0) {
        throw new BadRequestException('You have exceeded requests quota');
      }
      ...
      await fetch('https://api.brevo.com/v3/smtp/email', {
        ...
        body: JSON.stringify({
          sender: {
            name: 'Screen List Support',
            email: 'support@screenlist.co.za',
          },
          to: [{ email: email }],
  ...
  async findUserByUsername(username: string) {
    ...
    try {
      ...
      delete user.customerCode;
      const photoKey = this.db.key([
        'User',
        user[this.db.KEY]['name'],
        'UserPhoto',
        '0',
      ]);
      const coverKey = this.db.key([
        'User',
        user[this.db.KEY]['name'],
        'UserPhoto',
        '1',
      ...
      let [films] = await this.db
        .createQuery('History')
        .filter('xUser', '=', uid)
        .filter('xKind', '=', 'Film')
        .filter('xAction', '=', 'create')
        .order('xTimestamp', { descending: true })
        .limit(34)
        .run();
      let [companies] = await this.db
        .createQuery('History')
        .filter('xUser', '=', uid)
  ...
  async uploadProfilePhoto(opt: ImageOpt, image: Express.Multer.File) {
    try {
      if (opt.imageId !== '0') {
        throw new BadRequestException('Unknown index');
      }
      const data = await this.storage.uploadProfilePhoto(image);
      const photoData: CreateDisplayPhotoDto = { ...data };
      const { entity, history } = await this.db.createUserPhotoEntity(
        photoData,
        opt,
      );
  ...
  async updateProfilePhoto(data: UpdateDisplayPhotoDto, opt: ImageOpt) {
    try {
      const { entity, history } = await this.db.updateUserPhotoEntity(
        data,
        opt,
      );
        ...
        result.map(async (item) => {
          try {
            const [reviews] = await this.db
              .createQuery('Rating')
              .filter('authorUid', '=', item.uid)
              .order('created', { descending: true })
              .run();
            const recentReviews = reviews.filter(
              (val) => Number(new Date(val.created)) > Number(aYearAgo),
            );
  ...
  async approveToSetJournalist(opt: RequestOpt) {
    ...
    try {
      ...
      const journalist = await this.db.updateUserEntity(
        journalistData,
        userOptions,
      );
  ...
  async createJournalistInfo(data: CreateJournalistInfoDto, opt: UserOpt) {
    try {
      const { entity, history } = await this.db.createJournalistInfoEntity(
        data,
        opt,
      );
  ...
  async updateJournalistInfo(data: UpdateJournalistInfoDto, opt: UserOpt) {
    try {
      const { entity, history } = await this.db.updateJournalistInfoEntity(
        data,
        opt,
      );
  ...
  async checkEmptyThrone(opt: UserOpt) {
    ...
    try {
      ...
      if (requests.length > 100) {
        throw new NotFoundException('Daily chance already used');
      }
  ...
  // Other methods
  async voteToSetRole(opt: VoteOpt, role: 'admin' | 'curator' | 'moderator') {
    ...
    try {
      ...
      if (role == 'admin' || role == 'curator') {
        ...
        if (currentUser.role == 'admin') {
          votes.adminsTotalPoints = votes.adminsTotalPoints + 10;
          totalVotingPoints =
            votes.adminsTotalPoints +
            votes.curatorsTotalPoints +
            votes.moderatorsTotalPoints;
  ...
  async proposeRoleVote(opt: VoteOpt, role: 'admin' | 'curator' | 'moderator') {
    const currentUserKey = this.db.key(['User', opt.user]);
    try {
      const [result] = await this.db.get(currentUserKey);
      const admins = await this.findAllAdmins();
      ...
      if (role == 'admin' || role == 'curator') {
        ...
        const { entity, history } = await this.db.createVotesEntity(
          voteEntity,
          opt,
        );
        return { status: 'created' };
        ...
        const approvalPoints =
          (totalAdmins * 10 + totalCurators * 5 + totalModerators * 2) * 0.6;
        const voteEntity: CreateVotesDto = {
